// https://github.com/unjs/nitro/blob/main/src/core/scan.ts
        ...
        for (const scannedTask of scannedTasks) {
            if (scannedTask.name in nitro.options.tasks) {
                if (!nitro.options.tasks[scannedTask.name].handler) {
                    nitro.options.tasks[scannedTask.name].handler =
                        scannedTask.handler;
                }
            } else {
...
export async function scanHandlers(nitro: Nitro) {
    ...
    const handlersArray = await Promise.all([
        scanServerRoutes(
            nitro,
    ...
    const handlers = handlersArray.flat();

    const handlerMap = new Map<string, any>();
    for (const h of handlers) {
        const key = `${h.route}-${h.method || ""}-${h.env || ""}`;
        if (!handlerMap.has(key)) {
            handlerMap.set(key, h);
        }
    }

    nitro.scannedHandlers = [...middleware, ...handlerMap.values()];
